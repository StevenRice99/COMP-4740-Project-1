...
def main(layers: int, width: int, batch: bool, dropout: bool, augment: bool, epochs: int, load: bool):
    ...
    if epoch == 1:
        ...
        f.write("Epoch,Loss,Training Accuracy,Testing Accuracy,Training Precision,Testing Precision")
        f.close()
    train_accuracy, train_precision = test(model, 64, training)
    trainable_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Network parameters: {trainable_parameters}")
    save(name, model, best_model, epoch, no_change, best_accuracy, best_precision, loss)
