...
class NeuralNetwork(nn.Module):
    ...
    def __init__(self, hidden_number: int, hidden_size: int, batch_normalization: bool, dropout: bool):
        ...
        self.layers.append(
            nn.Linear(24 if hidden_number == 0 else hidden_size, 4))
        self.layers.append(nn.ReLU())
        # Define the loss.
...
def save(name: str, model, best_model, epoch: int, no_change: int, best_accuracy: float, best_precision: float, loss: float):
    """
    Save the model.
    ...
    :param best_precision: The best precision of the best model.
...
def write_parameters(name: str, best_accuracy: float, train_accuracy: float, best_precision: float, train_precision: float, inference_time: float, trainable_parameters: int, best_epoch: int):
    """
    ...
    :param best_precision: The best precision the model achieved.
    :param train_precision: The training precision.
    ...
    f.write(f"Testing Accuracy: {best_accuracy}\n"
            ...
            f"Testing Precision: {best_precision}\n"
...
def main(layers: int, width: int, batch: bool, dropout: bool, augment: bool, epochs: int, load: bool):
    ...
    print(f"Running on GPU with CUDA {torch.version.cuda}." if torch.cuda.is_available(
    ) else "Running on CPU.")
    ...
    df[24] = df[24].map({"Move-Forward": 0, "Slight-Right-Turn": 1,
                        "Sharp-Right-Turn": 2, "Slight-Left-Turn": 3})
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        df.iloc[:, 0:24], df.iloc[:, 24], test_size=0.20, random_state=1)
    # Create the datasets.
    ...
    if load:
        ...
        try:
            ...
            print(
                "Model to load has different structure than 'model_builder'.py, cannot load.")
            return
        train_accuracy, train_precision = test(model, 64, training)
        start = time.time_ns()
        accuracy, precision = test(model, 64, testing)
        end = time.time_ns()
        inference_time = ((end - start) / testing_total) / 1e+6
        print(f"Testing Accuracy = {accuracy}%\n"
              f"Training Accuracy = {train_accuracy}%\n"
              f"Testing Percision = {precision}%\n"
              f"Training Percision = {train_precision}%\n"
              f"Average Inference Time: {inference_time} ms")
        return
    ...
    if os.path.exists(f"{os.getcwd()}/Models/{name}/Model.pt"):
        try:
            print(
                f"Model '{name}' already exists, attempting to load to continue training...")
            saved = torch.load(f"{os.getcwd()}/Models/{name}/Model.pt")
            ...
            best_precision = saved['Best Precision']
            loss = saved['Loss']
            print(
                f"Continuing training for '{name}' from epoch {epoch} for {epochs} epochs.")
        except:
    ...
    write_parameters(name, best_accuracy, train_accuracy, best_precision, train_precision,
                     inference_time, trainable_parameters, 0)
    save(name, model, best_model, epoch, no_change,
         best_accuracy, best_precision, loss)
    ...
    while True:
        ...
        msg = f"Epoch {epoch}/{epochs} | {loss_message} | Train = {train_accuracy:.4}% {train_precision:.4}% | Test = {accuracy:.4}% {precision:.4}% | Best = {best_accuracy:.4}% {best_precision:.4}%  | {improvement}"
        ...
        if accuracy > best_accuracy:
            ...
            no_change = 0
            write_parameters(name, best_accuracy, train_accuracy, best_precision, train_precision,
                             inference_time, trainable_parameters, epoch)
        else:
            no_change += 1
        # Save data.
        f = open(f"{os.getcwd()}/Models/{name}/Training.csv", "a")
        f.write(
            f"\n{epoch},{loss},{train_accuracy},{accuracy}{train_precision},{precision}")
        f.close()
        epoch += 1
        save(name, model, best_model, epoch, no_change,
             best_accuracy, best_precision, loss)
...
if __name__ == '__main__':
    try:
        parser = argparse.ArgumentParser(
            formatter_class=argparse.RawDescriptionHelpFormatter, description="Wall-Following Robot Navigation")
        parser.add_argument("-l", "--layers", type=int,
                            help="The number of hidden layers.", default=6)
        parser.add_argument("-w", "--width", type=int,
                            help="The number of neurons in the hidden layers.", default=128)
        parser.add_argument(
            "-b", "--batch", help="Apply batch normalization to hidden layers.", action="store_true")
        parser.add_argument(
            "-d", "--dropout", help="Apply dropout to hidden layers.", action="store_true")
        parser.add_argument(
            "-a", "--augment", help="Augment the training data.", action="store_true")
        parser.add_argument("-e", "--epoch", type=int,
                            help="The number of epochs to train for.", default=100)
        parser.add_argument(
            "-t", "--test", help="Test the model without doing any training.", action="store_true")
        a = vars(parser.parse_args())
        main(a["layers"], a["width"], a["batch"],
             a["dropout"], a["augment"], a["epoch"], a["test"])
